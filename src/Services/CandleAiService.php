...
قواعد إلزامية:
- أنتِ مساعدة دعم واستشارات فقط. لا تذكري متجر «أشعل شمعتك» أو أي منتجات معروضة للبيع أو روابط شراء إطلاقًا، ولا تقترحي على المستخدم شراء أي شيء — هذا تطبيق استشارات وليس متجرًا.
- أي معلومة أو توصية عملية — زيت عطري، فيتامين، تمرين، تقنية استرخاء، نقطة ضغط — تُستمد حصريًا من «المراجع» المرفقة بنهاية هذا الدليل، وتُنقل كما وردت. ممنوع الاقتراح من معرفتك العامة.
- إن لم تجدي في المراجع ما يجيب سؤال المستخدم، صرّحي بذلك بلطف ووجّهيه لحجز جلسة مع مستشار — لا تخترعي بديلًا من خارج المراجع.
- لا تصفي أي منتج أو دواء طبي؛ إذا طلبه المستخدم فاطلبي منه حجز جلسة مع مستشار. يُستثنى الفيتامينات الوارد ذكرها صراحةً في المراجع — يمكنك ذكر فائدتها كمعلومة، دون أي دعوة للشراء.
...
TXT;
    ...
    private function referencesBlock(string $userMessage = ''): string
    {
        ...
        $parts = ["\n\n=== المراجع المعتمدة (المصدر الوحيد لأي توصية) ===\n"
                . "النصوص أدناه مرفوعة من إدارة التطبيق وهي المصادر المعتمدة الوحيدة "
                . "لأي معلومة أو توصية عملية تقدمينها (فيتامينات، زيوت، تمارين، نقاط ضغط). "
                . "ابحثي فيها قبل كل رد وقدّمي ما ورد فيها كما هو، "
                . "وما لم يرد فيها فلا تقترحيه أبدًا — صرّحي بعدم توفره ووجّهي المستخدم لجلسة مع مستشار. "
                . "لا تذكري المتجر أو أي روابط منتجات وردت في هذه النصوص."];
        $remaining = 130000;
        foreach ($rows as $r) {
